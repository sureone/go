...
int http_request_parse(server *srv, connection *con) {
	...
	if (con->conf.log_request_header) {
		log_error_write( __FILE__, __LINE__, "sdsdSb",
				"fd:", con->fd,
	...
	for (i = 0, first = 0; i < con->parse_request->used && line == 0; i++) {
			...
			if (con->parse_request->ptr[i+1] == '\n') {
				...
				if (request_line_stage != 2) {
					...
					if (srv->srvconf.log_request_header_on_error) {
						log_error_write( __FILE__, __LINE__, "s", "incomplete request line -> 400");
				...
				*(proto - 1) = '\0';
				...
				if (-1 == (r = get_http_method_key(method))) {
					...
					if (srv->srvconf.log_request_header_on_error) {
						log_error_write( __FILE__, __LINE__, "s", "unknown http-method -> 501");
						log_error_write( __FILE__, __LINE__, "Sb",
								"request-header:\n",
								con->request.request);
					}
				...
				if (0 == strncmp(proto, "HTTP/", sizeof("HTTP/") - 1)) {
					...
					if (invalid_version) {
						...
						if (srv->srvconf.log_request_header_on_error) {
							log_error_write( __FILE__, __LINE__, "s", "unknown protocol -> 400");
							log_error_write( __FILE__, __LINE__, "Sb",
									"request-header:\n",
									con->request.request);
						}
					...
					if (major_num == 1 && minor_num == 1) {
						...
						if (srv->srvconf.log_request_header_on_error) {
							log_error_write( __FILE__, __LINE__, "s", "unknown HTTP version -> 505");
							log_error_write( __FILE__, __LINE__, "Sb",
									"request-header:\n",
									con->request.request);
						}
		...
		case ' ':
			...
			default:
				...
				if (srv->srvconf.log_request_header_on_error) {
					log_error_write( __FILE__, __LINE__, "s", "overlong request line -> 400");
					log_error_write( __FILE__, __LINE__, "Sb",
							"request-header:\n",
							con->request.request);
				}
	...
	if (con->request.uri->used == 1) {
		...
		if (srv->srvconf.log_request_header_on_error) {
			log_error_write( __FILE__, __LINE__, "s", "no uri specified -> 400");
			log_error_write( __FILE__, __LINE__, "Sb",
							"request-header:\n",
							con->request.request);
		}
	...
	for (; i < con->parse_request->used && !done; i++) {
		...
		if (is_key) {
			...
			case '}':
				...
				log_error_write( __FILE__, __LINE__, "sbsds",
			...
			case '\t':
				...
				for (j = 1; !got_colon; j++) {
					...
					default:
						...
						if (srv->srvconf.log_request_header_on_error) {
							log_error_write( __FILE__, __LINE__, "s", "WS character in key -> 400");
							log_error_write( __FILE__, __LINE__, "Sb",
								"request-header:\n",
								con->request.request);
						}
			...
			case '\r':
				if (con->parse_request->ptr[i+1] == '\n' && i == first) {
					...
					if (srv->srvconf.log_request_header_on_error) {
						log_error_write( __FILE__, __LINE__, "s", "CR without LF -> 400");
						log_error_write( __FILE__, __LINE__, "Sb",
							"request-header:\n",
							con->request.request);
					}
				...
				if (con->parse_request->ptr[i+1] == '\n') {
					...
					if (in_folding) {
						...
						if (!key || !key_len) {
							...
							if (srv->srvconf.log_request_header_on_error) {
								log_error_write( __FILE__, __LINE__, "s", "WS at the start of first line -> 400");
	...
	if (con->request.http_version == HTTP_VERSION_1_1) {
		...
		if (con->request.http_host == NULL ||
			...
			if (srv->srvconf.log_request_header_on_error) {
				log_error_write( __FILE__, __LINE__, "s", "HTTP/1.1 but Host missing -> 400");
				log_error_write( __FILE__, __LINE__, "Sb",
						"request-header:\n",
						con->request.request);
			}
	...
	if (con_length_set) {
		...
		if (con->request.content_length > SSIZE_MAX) {
			...
			log_error_write( __FILE__, __LINE__, "sos",
