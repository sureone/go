...
static int network_ssl_servername_callback(SSL *ssl, int *al, server *srv) {
	...
	if (NULL == con->conf.ssl_ctx) {
		...
		log_error_write(__FILE__, __LINE__, "ssb", "SSL:",
...
static int network_server_init(server *srv, buffer *host_token, specific_config *s) {
	...
	if (NULL == (sp = strrchr(b->ptr, ':'))) {
		log_error_write(__FILE__, __LINE__, "sb", "value of $SERVER[\"socket\"] has to be \"ip:port\".", b);
	...
	*(sp++) = '\0';
	...
	if (host[0] == '/') {
		...
		log_error_write(__FILE__, __LINE__, "sd", "port out of range:", port);
	...
	if (is_unix_domain_socket) {
		...
		if (-1 == (srv_socket->fd = socket(srv_socket->addr.plain.sa_family, SOCK_STREAM, 0))) {
			log_error_write(__FILE__, __LINE__, "ss", "socket failed:", strerror(errno));
			goto error_free_socket;
		}
#else
	...
	if (setsockopt(srv_socket->fd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) < 0) {
		log_error_write(__FILE__, __LINE__, "ss", "socketsockopt(SO_REUSEADDR) failed:", strerror(errno));
		goto error_free_socket;
	}
	...
	case AF_INET6:
		...
		if (host == NULL) {
			...
			log_error_write(__FILE__, __LINE__, "s", "warning: please use server.use-ipv6 only for hostnames, not without server.bind / empty address; your config will break if the kernel default for IPV6_V6ONLY changes");
		} else {
			...
			if (s->set_v6only) {
				...
				if (-1 == setsockopt(srv_socket->fd, IPPROTO_IPV6, IPV6_V6ONLY, &val, sizeof(val))) {
					log_error_write(__FILE__, __LINE__, "ss", "socketsockopt(IPV6_V6ONLY) failed:", strerror(errno));
					goto error_free_socket;
				}
			} else {
				log_error_write(__FILE__, __LINE__, "s", "warning: server.set-v6only will be removed soon, update your config to have different sockets for ipv4 and ipv6");
			}
	...
	case AF_INET:
		...
		if (host == NULL) {
			...
			if (he->h_addrtype != AF_INET) {
				log_error_write(__FILE__, __LINE__, "sd", "addr-type != AF_INET: ", he->h_addrtype);
				goto error_free_socket;
			}

			if (he->h_length != sizeof(struct in_addr)) {
				log_error_write(__FILE__, __LINE__, "sd", "addr-length != sizeof(in_addr): ", he->h_length);
				goto error_free_socket;
			}
	...
	if (0 != bind(srv_socket->fd, (struct sockaddr *) &(srv_socket->addr), addr_len)) {
		...
		case AF_UNIX:
			...
			break;
		default:
			log_error_write(__FILE__, __LINE__, "ssds",
					"can't bind to port:",
	...
	if (-1 == listen(srv_socket->fd, 128 * 8)) {
		log_error_write(__FILE__, __LINE__, "ss", "listen failed: ", strerror(errno));
		goto error_free_socket;
	}
	...
	if (s->is_ssl) {
		...
		if (NULL == (srv_socket->ssl_ctx = s->ssl_ctx)) {
			log_error_write(__FILE__, __LINE__, "s", "ssl.pemfile has to be set");
			goto error_free_socket;
		}
#else
		...
		if (-1 == setsockopt(srv_socket->fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &v, sizeof(v))) {
			log_error_write(__FILE__, __LINE__, "ss", "can't set TCP_DEFER_ACCEPT: ", strerror(errno));
		}
#endif
	} else {
		...
		if (setsockopt(srv_socket->fd, SOL_SOCKET, SO_ACCEPTFILTER, &afa, sizeof(afa)) < 0) {
			if (errno != ENOENT) {
				log_error_write(__FILE__, __LINE__, "ss", "can't set accept-filter 'httpready': ", strerror(errno));
			}
		}
#endif
	}
...
# define SSL_OP_NO_COMPRESSION 0
		...
		if (buffer_is_empty(s->ssl_pemfile)) goto _skip_ssl;
		...
		if (!buffer_is_empty(s->ssl_dh_file)) {
			...
			if (bio == NULL) {
				log_error_write(__FILE__, __LINE__, "ss", "SSL: Unable to open file", s->ssl_dh_file->ptr);
				return -1;
			}
			dh = PEM_read_bio_DHparams(bio, NULL, NULL, NULL);
			BIO_free(bio);
			if (dh == NULL) {
				log_error_write(__FILE__, __LINE__, "ss", "SSL: PEM_read_bio_DHparams failed", s->ssl_dh_file->ptr);
				return -1;
			}
		} else {
			...
			dh = DH_new();
			if (dh == NULL) {
				log_error_write(__FILE__, __LINE__, "s", "SSL: DH_new () failed");
				return -1;
			}
			...
			if ((dh->p == NULL) || (dh->g == NULL)) {
				DH_free(dh);
				log_error_write(__FILE__, __LINE__, "s", "SSL: BN_bin2bn () failed");
				return -1;
			}
		}
		...
		if (!buffer_is_empty(s->ssl_ec_curve)) {
			...
			if (nid == 0) {
				log_error_write(__FILE__, __LINE__, "ss", "SSL: Unknown curve name", s->ssl_ec_curve->ptr);
				return -1;
			}
		} else {
			/* Default curve */
		...
		if (ecdh == NULL) {
			log_error_write(__FILE__, __LINE__, "ss", "SSL: Unable to create curve", s->ssl_ec_curve->ptr);
			return -1;
		}
		...
		if (SSL_CTX_check_private_key(s->ssl_ctx) != 1) {
			log_error_write(__FILE__, __LINE__, "sssb", "SSL:",
...
_skip_ssl:
	...
	fprintf(stderr, "server start at %s\n", b->ptr );
	...
	if (!buffer_is_empty(srv->srvconf.network_backend)) {
		...
		if (NULL == network_backends[i].name) {
			...
			log_error_write(__FILE__, __LINE__, "sb",
...
int network_write_chunkqueue(server *srv, connection *con, chunkqueue *cq, off_t max_bytes) {
	...
	if (srv_socket->is_ssl) {
		...
		debug_log("sdd", "write data to",con,con->fd);
