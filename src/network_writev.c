...
#define GByte * 1024 MByte
			...
			if (c->file.mmap.start == MAP_FAILED ||
				...
				if (-1 == c->file.fd) {  /* open the file if not already open */
					if (-1 == (c->file.fd = open(c->file.name->ptr, O_RDONLY))) {
						log_error_write( __FILE__, __LINE__, "sbs", "open failed for:", c->file.name, strerror(errno));
				...
				if (MAP_FAILED == (c->file.mmap.start = mmap(0, to_mmap, PROT_READ, MAP_SHARED, c->file.fd, c->file.mmap.offset))) {
					...
					log_error_write( __FILE__, __LINE__, "ssbd", "mmap failed:",
			...
			if (toSend < 0) {
				log_error_write( __FILE__, __LINE__, "soooo",
						"toSend is negative:",
						toSend,
						c->file.mmap.length,
						abs_offset,
