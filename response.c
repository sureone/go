...
int http_response_write_header(server *srv, connection *con) {
	...
	if (con->conf.log_response_header) {
		log_error_write( __FILE__, __LINE__, "sSb", "Response-Header:", "\n", b);
	}
...
handler_t http_response_prepare(server *srv, connection *con) {
	...
	if (con->mode == DIRECT && con->physical.path->used == 0) {
		...
		if (con->conf.log_condition_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "run condition");
		}
		...
		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- splitting Request-URI");
			log_error_write( __FILE__, __LINE__,  "sb", "Request-URI  : ", con->request.uri);
			log_error_write( __FILE__, __LINE__,  "sb", "URI-scheme   : ", con->uri.scheme);
			log_error_write( __FILE__, __LINE__,  "sb", "URI-authority: ", con->uri.authority);
			log_error_write( __FILE__, __LINE__,  "sb", "URI-path     : ", con->uri.path_raw);
			log_error_write( __FILE__, __LINE__,  "sb", "URI-query    : ", con->uri.query);
		}
		...
		case HANDLER_ERROR:
			return r;
		default:
			log_error_write( __FILE__, __LINE__, "sd", "handle_uri_raw: unknown return value", r);
			break;
		}
		...
		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- sanatising URI");
			log_error_write( __FILE__, __LINE__,  "sb", "URI-path     : ", con->uri.path);
		}
		...
		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- before doc_root");
			log_error_write( __FILE__, __LINE__,  "sb", "Doc-Root     :", con->physical.doc_root);
			log_error_write( __FILE__, __LINE__,  "sb", "Rel-Path     :", con->physical.rel_path);
			log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
		}
		...
		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- after doc_root");
			log_error_write( __FILE__, __LINE__,  "sb", "Doc-Root     :", con->physical.doc_root);
			log_error_write( __FILE__, __LINE__,  "sb", "Rel-Path     :", con->physical.rel_path);
			log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
		}
		...
		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- logical -> physical");
			log_error_write( __FILE__, __LINE__,  "sb", "Doc-Root     :", con->physical.doc_root);
			log_error_write( __FILE__, __LINE__,  "sb", "Rel-Path     :", con->physical.rel_path);
			log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
		}
	...
	if (con->mode == DIRECT) {
		...
		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- handling physical path");
			log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
		}
		...
		if (HANDLER_ERROR != stat_cache_get_entry(srv, con, con->physical.path, &sce)) {
			...
			if (con->conf.log_request_handling) {
				log_error_write( __FILE__, __LINE__,  "s",  "-- file found");
				log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
			}
#ifdef HAVE_LSTAT
			if ((sce->is_symlink != 0) && !con->conf.follow_symlink) {
				...
				if (con->conf.log_request_handling) {
					log_error_write( __FILE__, __LINE__,  "s",  "-- access denied due symlink restriction");
					log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
				}
			...
			case EACCES:
				...
				if (con->conf.log_request_handling) {
					log_error_write( __FILE__, __LINE__,  "s",  "-- access denied");
					log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
				}
			...
			case ENOENT:
				...
				if (con->conf.log_request_handling) {
					log_error_write( __FILE__, __LINE__,  "s",  "-- file not found");
					log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
				}
			...
			default:
				...
				log_error_write( __FILE__, __LINE__, "ssbsb",
			...
			if (found == 0) {
				...
				if (con->conf.log_file_not_found) {
					log_error_write( __FILE__, __LINE__, "sbsb",
			...
			if (con->conf.log_request_handling) {
				log_error_write( __FILE__, __LINE__,  "s",  "-- after pathinfo check");
				log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
				log_error_write( __FILE__, __LINE__,  "sb", "URI          :", con->uri.path);
				log_error_write( __FILE__, __LINE__,  "sb", "Pathinfo     :", con->request.pathinfo);
			}
		}

		if (con->conf.log_request_handling) {
			log_error_write( __FILE__, __LINE__,  "s",  "-- handling subrequest");
			log_error_write( __FILE__, __LINE__,  "sb", "Path         :", con->physical.path);
		}
		...
		default:
			if (con->conf.log_request_handling) {
				log_error_write( __FILE__, __LINE__,  "s",  "-- subrequest finished");
			}
