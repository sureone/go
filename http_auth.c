...
static int http_auth_get_password(server *srv, mod_auth_plugin_data *p, buffer *username, buffer *realm, buffer *password) {
	...
	if (p->conf.auth_backend == AUTH_BACKEND_HTDIGEST) {
		...
		if (0 != stream_open(&f, p->conf.auth_htdigest_userfile)) {
			log_error_write( __FILE__, __LINE__, "sbss", "opening digest-userfile", p->conf.auth_htdigest_userfile, "failed:", strerror(errno));
		...
		if (0 != stream_open(&f, auth_fn)) {
			log_error_write( __FILE__, __LINE__, "sbss",
...
static int http_auth_match_rules(server *srv, mod_auth_plugin_data *p, const char *url, const char *username, const char *group, const char *host) {
...
#if 0
	log_error_write( __FILE__, __LINE__, "sb", "rules", require->value);
#endif
	...
	while (1) {
		...
		if (k_len == 4) {
			if (0 == strncmp(k, "user", k_len)) {
				...
				log_error_write( __FILE__, __LINE__, "s", "host ... (not implemented)");
			} else {
				log_error_write( __FILE__, __LINE__, "s", "unknown key");
				return -1;
			}
		} else if (k_len == 5) {
			if (0 == strncmp(k, "group", k_len)) {
				log_error_write( __FILE__, __LINE__, "s", "group ... (not implemented)");
			} else {
				log_error_write( __FILE__, __LINE__, "ss", "unknown key", k);
				return -1;
			}
		} else {
			log_error_write( __FILE__, __LINE__, "s", "unknown  key");
	...
	log_error_write( __FILE__, __LINE__, "s", "nothing matched");
	...
	if (p->conf.auth_backend == AUTH_BACKEND_HTDIGEST) {
		...
		for (i = 0; i < username->used - 1; i++) {
			...
			if (!isalpha(c) &&
				...
				log_error_write( __FILE__, __LINE__, "sbd",
		...
		if (p->anon_conf->ldap == NULL ||
			...
			if (p->anon_conf->ldap == NULL || ret != LDAP_SERVER_DOWN ||
				...
				if (p->anon_conf->ldap == NULL ||
				    LDAP_SUCCESS != (ret = ldap_search_s(p->anon_conf->ldap, p->conf.auth_ldap_basedn->ptr, LDAP_SCOPE_SUBTREE, p->ldap_filter->ptr, attrs, 0, &lm))) {
					log_error_write( __FILE__, __LINE__, "sssb",
		...
		if (NULL == (first = ldap_first_entry(p->anon_conf->ldap, lm))) {
			log_error_write( __FILE__, __LINE__, "s", "ldap ...");
		...
		if (NULL == (ldap = ldap_init(p->conf.auth_ldap_hostname->ptr, LDAP_PORT))) {
			log_error_write( __FILE__, __LINE__, "ss", "ldap ...", strerror(errno));
			return -1;
		}

		ret = LDAP_VERSION3;
		if (LDAP_OPT_SUCCESS != (ret = ldap_set_option(ldap, LDAP_OPT_PROTOCOL_VERSION, &ret))) {
			log_error_write( __FILE__, __LINE__, "ss", "ldap:", ldap_err2string(ret));
		...
		if (p->conf.auth_ldap_starttls == 1) {
	 		if (LDAP_OPT_SUCCESS != (ret = ldap_start_tls_s(ldap, NULL,  NULL))) {
	 			log_error_write( __FILE__, __LINE__, "ss", "ldap startTLS failed:", ldap_err2string(ret));
...
int http_auth_basic_check(server *srv, connection *con, mod_auth_plugin_data *p, array *req, buffer *url, const char *realm_str) {
	...
	if (!base64_decode(username, realm_str)) {
		log_error_write( __FILE__, __LINE__, "sb", "decodeing base64-string failed", username);
	...
	if (NULL == (pw = strchr(username->ptr, ':'))) {
		log_error_write( __FILE__, __LINE__, "sb", ": is missing in", username);
	...
	*pw++ = '\0';
	...
	if (http_auth_get_password(srv, p, username, realm->value, password)) {
		...
		if (AUTH_BACKEND_UNSET == p->conf.auth_backend) {
			log_error_write( __FILE__, __LINE__, "s", "auth.backend is not set");
		} else {
			log_error_write( __FILE__, __LINE__, "ss", "get_password failed, IP:", inet_ntop_cache_get_ip(srv, &(con->dst_addr)));
		}
	...
	if (http_auth_basic_password_compare(srv, p, req, username, realm->value, password, pw)) {
		log_error_write( __FILE__, __LINE__, "sbsBss", "password doesn't match for", con->uri.path, "username:", username, ", IP:", inet_ntop_cache_get_ip(srv, &(con->dst_addr)));
	...
	if (http_auth_match_rules(srv, p, url->ptr, username->ptr, NULL, NULL)) {
		...
		log_error_write( __FILE__, __LINE__, "s", "rules didn't match");
...
#undef S
	...
	if (p->conf.auth_debug > 1) {
		log_error_write( __FILE__, __LINE__, "ss", "username", username);
		log_error_write( __FILE__, __LINE__, "ss", "realm", realm);
		log_error_write( __FILE__, __LINE__, "ss", "nonce", nonce);
		log_error_write( __FILE__, __LINE__, "ss", "uri", uri);
		log_error_write( __FILE__, __LINE__, "ss", "algorigthm", algorithm);
		log_error_write( __FILE__, __LINE__, "ss", "qop", qop);
		log_error_write( __FILE__, __LINE__, "ss", "cnonce", cnonce);
		log_error_write( __FILE__, __LINE__, "ss", "nc", nc);
		log_error_write( __FILE__, __LINE__, "ss", "response", respons);
	}
