...
public class WeiboThread extends Activity {
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
				...
				case GoController.RSP_POST_REPLY:
					onPostDone(msg.arg1);
				break;
				case GoController.RSP_RM_REPLY:{					
					int pos=0;
    ...
    void post(String txt) {
		...
		GoApp.getInstance().getGoController().postReply(GoApp.getInstance().mCurrentThread.id,txt);
    }
	...
	void loadMoreNew(){
		...
		GoApp.getInstance().getGoController().loadThreadsFrom(mMaxDt,GoApp.getInstance().mCurrentThread.id);
	}
	
	void loadMoreOld(){
		if(mMinDt==null)
			GoApp.getInstance().getGoController().loadThreadsTo("now",GoApp.getInstance().mCurrentThread.id);
		else
			GoApp.getInstance().getGoController().loadThreadsTo(mMinDt,GoApp.getInstance().mCurrentThread.id);	
	}
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        try {
            ...
            if(menuItemIndex==0) { //delete
				if(info.position>0){
					MessageThread mt=mMainMsgThreads.get(info.position-1);
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        if (v.getId()==R.id.threadList) {
            ...
            menu.setHeaderTitle(this.getString(R.string.SgfMenu));
    ...
    void onDeleteThread(int id,int uid) {
		...
		GoApp.getInstance().getGoController().removeReply(id,uid);
    ...
    public class AppsAdapter extends BaseAdapter implements AdapterView.OnItemClickListener,
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            MessageThread mt=GoApp.getInstance().mCurrentThread;
			
			if(position >0){
				mt=mMainMsgThreads.get(position-1);
			}
            holder.txtContent.setText(mt.content);
            holder.txtAuthor.setText(mt.uname+
			WeiboThread.this.getString(R.string.postat)+getTimeDiff(mt.cdate));
			
			if(position==0){
				holder.txtReplyAndView.setText(WeiboThread.this.getString(R.string.reply)+" "+mt.rcount);
			}else{
				holder.txtReplyAndView.setText("");
			}
