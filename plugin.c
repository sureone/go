...
#define PLUGIN_INIT(x)\
	p = plugin_init(); \
	if (x ## _plugin_init(p)) { \
		log_error_write( __FILE__, __LINE__, "ss", #x, "plugin init failed" ); \
		plugin_free(p); \
		return -1;\
	}\
	...
	for (i = 0; i < srv->srvconf.modules->used; i++) {
		...
		for (j = 0; j < i; j++) {
			if (buffer_is_equal(d->value, ((data_string *) srv->srvconf.modules->data[j])->value)) {
				log_error_write( __FILE__, __LINE__, "sbs", "Cannot load plugin", d->value, "more than once, please fix your config (we may not accept such configs in future releases");
				continue;
			}
		}
		...
		if (NULL == (p->lib = LoadLibrary(srv->tmp_buf->ptr))) {
			...
			log_error_write( __FILE__, __LINE__, "ssb", "LoadLibrary() failed",
		...
		if (NULL == (p->lib = dlopen(srv->tmp_buf->ptr, RTLD_NOW|RTLD_GLOBAL))) {
			log_error_write( __FILE__, __LINE__, "sbs", "dlopen() failed for:",
					srv->tmp_buf, dlerror());
		...
		if (init == NULL)  {
			...
			log_error_write( __FILE__, __LINE__, "sbs", "getprocaddress failed:", srv->tmp_buf, lpMsgBuf);
		...
		*(void **)(&init) = dlsym(p->lib, srv->tmp_buf->ptr);
		...
		if ((error = dlerror()) != NULL)  {
			log_error_write( __FILE__, __LINE__, "s", error);
...
#endif
		if ((*init)(p)) {
			log_error_write( __FILE__, __LINE__, "ss", modules, "plugin init failed" );

			plugin_free(p);
			return -1;
		}
#if 0
		log_error_write( __FILE__, __LINE__, "ss", modules, "plugin loaded" );
#endif
		plugins_register(srv, p);
	}
	...
	handler_t plugins_call_##y(server *srv, connection *con) {\
		...
		for (j = 0; j < srv->plugins.used && slot[j]; j++) { \
			...
			case HANDLER_ERROR:\
				return r;\
			default:\
				log_error_write( __FILE__, __LINE__, "sbs", #x, p->name, "unknown state");\
	...
	handler_t plugins_call_##y(server *srv) {\
		...
		for (j = 0; j < srv->plugins.used && slot[j]; j++) { \
			...
			case HANDLER_ERROR:\
				return r;\
			default:\
				log_error_write( __FILE__, __LINE__, "sbsd", #x, p->name, "unknown state:", r);\
				return HANDLER_ERROR;\
			}\
		}\
		return HANDLER_GO_ON;\
